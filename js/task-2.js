...
images.forEach(image => {
  const photoList = document.createElement('li'); // tworzenie elementu li
  const img = document.createElement('img');  // tworzenie elementu img
  img.src = image.url; //skad pobrac sciezke
  img.alt = image.alt;  // skad pobrac opis
  console.log(photoList);
  console.log(img);

  photoList.appendChild(img) // dodanie img do utworzonej listy 
  gallery.appendChild(photoList)// dodanie listy do ul gallery
});
